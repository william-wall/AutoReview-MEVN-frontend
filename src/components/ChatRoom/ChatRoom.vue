      ...
      <b-list-group class="panel-body" v-chat-scroll>
        <b-list-group-item v-for="(item, index) in chats" class="chat">
          <div class="left clearfix" v-if="item.nickname === nickname">
            <b-img left src="https://www.qualiscare.com/wp-content/uploads/2017/08/default-user.png" rounded="circle" width="75" height="75" alt="img" class="m-1" />
          ...
          <div class="right clearfix" v-else>
            <b-img right src="https://freeiconshop.com/wp-content/uploads/edd/car-flat.png" rounded="circle" width="75" height="75" alt="img" class="m-1" />
            <div class="chat-body clearfix">
              <div class="header">
                <strong class="primary-font">{{ item.nickname }}</strong> <small class="pull-right text-muted">
                <span class="glyphicon glyphicon-time"></span>{{ item.created_date }}</small>
...
export default {
  ...
  created () {
    axios.get(`https://autoreview-backend.herokuapp.com/api/chats/` + this.$route.params.id)
    .then(response => {
